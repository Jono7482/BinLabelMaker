SETTINGS = {'FONT': '/data/arial.ttf',
            'DEFAULT': 'GLAZE-A...B-01...04-A...E',
            'LOGO': 'data/Laguna Logo.png',
            'GUI_SIZE': (800, 510),
            'PREFIX_B': '%B%',
            'PREFIX_L': '%LOC%',
            'SHOW_QR_DATA': True,
            'QR_DATA_SCALE': 2.5
            }
LABELS = {'Full Page': {
            'lpp': (1, 1),
            'canvas': (850, 1100),
            'canvas_usable': (850, 1100),
            'page_offset': (0, 20),
            'qr_size': 800,
            'qr_x': 25,
            'qr_y': 25,
            'text_x': 425,  # canvas width / 2
            'text_y': 845,
            'logo': 230,
            'logo_x': 280,  # canvas width / 2 - logo / 2
            'logo_y': 870,
            'font_size': 100,
        },
        'Rack Labels': {
            'lpp': (1, 3),
            'canvas': (800, 1100),
            'canvas_usable': (800, 1100),
            'page_offset': (0, 0),
            'qr_size': 265,
            'qr_x': 50,
            'qr_y': 50,
            'text_x': 545,  # canvas width * .66
            'text_y': 100,
            'logo': 160,
            'logo_x': 448,  # canvas width * .66 - logo size / 2
            'logo_y': 160,
            'font_size': 50,
        },
        'Floor Labels': {
            'lpp': (2, 2),
            'canvas': (800, 1100),
            'canvas_usable': (800, 1100),
            'page_offset': (0, 0),
            'qr_size': 290,
            'qr_x': 50,
            'qr_y': 30,
            'text_x': 187,  # canvas width /4
            'text_y': 340,
            'logo': 170,
            'logo_x': 90,  # canvas width /4 - logo size / 2
            'logo_y': 355,
            'font_size': 50,
        },
        'SMALL 1X2': {
            'lpp': (4, 10),
            'canvas': (850, 1100),
            'canvas_usable': (820, 1040),
            'page_offset': (15, 45),
            'qr_size': 60,
            'qr_x': 15,
            'qr_y': 10,
            'text_x': 135,
            'text_y': 20,
            'logo': 40,
            'logo_x': 120,  # canvas width /4 - logo size / 2
            'logo_y': 35,
            'font_size': 20,
        },
        }

